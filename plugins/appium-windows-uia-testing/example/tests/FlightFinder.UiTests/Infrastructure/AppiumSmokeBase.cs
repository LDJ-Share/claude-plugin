...
//
// This is the appium-windows-uia-setup skill's genericized AppiumSmokeBase.cs
// with its placeholders resolved for this demo:
//   * namespace      -> FlightFinder.UiTests.Infrastructure
//   * AutomationName  -> "NovaWindows" (appium-novawindows-driver; no WinAppDriver)
//   * ResolveExePath  -> src/FlightFinder.App/bin/Debug/net10.0-windows/FlightFinder.App.exe
...
public abstract class AppiumSmokeBase
{
    ...
    public void BaseOneTimeSetUp()
    {
        ...
        {
            Assert.Fail($"App exe not found at {exePath} — build the solution first.");
            return;
        }
        ...
        if (!title.Contains("Flight Finder", StringComparison.OrdinalIgnoreCase))
        {
            _driver.Quit();
            _driver = null;
    ...
    private static string ResolveExePath()
    {
        ...
        for (int i = 0; i < 5; i++) // walk up to the sample root
            dir = dir.Parent ?? throw new DirectoryNotFoundException("Could not reach the sample root.");
        return Path.Combine(dir.FullName, "src", "FlightFinder.App", "bin", "Debug", "net10.0-windows", "FlightFinder.App.exe");
    }
